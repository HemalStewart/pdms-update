...
if (!function_exists('logged_in_user_type')) {
    ...
    function logged_in_user_type()
    {
        ...
        if ($CI->session->userdata('user_type')) :
            $logged_in_type = $CI->session->userdata('user_type');
        endif;
...
if (!function_exists('logged_in_user_role')) {
    ...
    function logged_in_user_role()
    {
        ...
        $ci = &get_instance();
        $ci->db->select('R.slug');
        $ci->db->from('roles AS R');
        $ci->db->where('R.id', $ci->session->userdata('role_id'));

        return @$ci->db->get()->row()->slug;
    }
}
...
if (!function_exists('get_routines_by_day')) {
    ...
    function get_routines_by_day($day, $class_id = null, $section_id = null)
    {
        ...
        $ci->db->select('R.*, S.sub_name AS subject, T.name AS teacher, C.School_className AS class_name');
        $ci->db->from('routines AS R');
        //  $ci->db->join('subjects AS S', 'S.id = R.subject_id', 'left');
        $ci->db->join('sub_list AS S', 'S.id = R.subject_id', 'left');
        $ci->db->join('teachers AS T', 'T.id = R.teacher_id', 'left');
        $ci->db->join('schoolclass AS C', 'C.School_classesid = R.class_id', 'left');
...
if (!function_exists('get_user_data_by_id')) {
    ...
    function get_user_data_by_id($user_id, $role_id = 0)
    {
        $ci = &get_instance();
        $ci->db->select('U.*,DD.*');
        ...
        if ($role_id == TEACHER) {
            $ci->db->join('teachers AS DD', 'U.id = DD.user_id', 'left');
        } elseif ($role_id == GUARDIAN) {
            $ci->db->join('guardians AS DD', 'U.id = DD.user_id', 'left');
        } else {
            $ci->db->join('department_dir AS DD', 'U.id = DD.user_id', 'left');
        }
...
if (!function_exists('get_user_by_id')) {
    ...
    function get_user_by_id($user_id)
    {
        ...
        if ($user->role_id == SUPER_ADMIN) {
            ...
            $ci->db->where('G.user_id', $user_id);
            return $ci->db->get()->row();
        } elseif ($user->role_id == PROVINCIAL) {

            $ci->db->select('G.*, U.username, U.role_id');
            $ci->db->from('provincial_dir AS G');
            $ci->db->join('users AS U', 'U.id = G.user_id', 'left');
            $ci->db->where('G.user_id', $user_id);
            return $ci->db->get()->row();
        } else {
...
if (!function_exists('get_responsibility')) {
    ...
    function get_responsibility()
    {
        ...
        return array(
            ...
            'Sinhala' => $ci->lang->line('Sinhala'),
            'Pali' => $ci->lang->line('Pali'),
            'Sanskrit' => $ci->lang->line('Sanskrit'),
            'Thripitaka dharmaya' => $ci->lang->line('Thripitaka dharmaya'),
            'Maths' => $ci->lang->line('Maths'),
            'English' => $ci->lang->line('English'),
            'Tamil' => $ci->lang->line('Tamil'),
            'Sociology' => $ci->lang->line('Sociology'),
            'Helth Science' => $ci->lang->line('Helth Science'),
            'History' => $ci->lang->line('History'),
            'Geography' => $ci->lang->line('Geography'),
            'Buddhism' => $ci->lang->line('Buddhism'),
            'Economics' => $ci->lang->line('Economics'),
            'Business Studies' => $ci->lang->line('Business Studies'),
            'Music' => $ci->lang->line('Music'),
            'Dansing' => $ci->lang->line('Dansing'),
            'Information Technology' => $ci->lang->line('Information Technology')

        );
    }
}
if (!function_exists('get_ordination_chapter')) {

    function get_ordination_chapter()
    {
        $ci = &get_instance();
        return array(
            'Siam Nikaya' => $ci->lang->line('Siam Nikaya'),
            'Amarapura Nikaya' => $ci->lang->line('Amarapura Nikaya'),
            'Ramanna Nikaya' => $ci->lang->line('Ramanna Nikaya')
        );
    }
}
...
if (!function_exists('get_religion')) {
    ...
    function get_religion()
    {
        $ci = &get_instance();
        return array(
            'Buddhist' => $ci->lang->line('Buddhist'),
...
if (!function_exists('get_type')) {
    ...
    function get_type()
    {
        $ci = &get_instance();
        return array(
            'පූජ්‍ය.' => $ci->lang->line('reverend1'),
            'සීලමාතා.' => $ci->lang->line('seelamatha1'),
            'මයා.' => $ci->lang->line('mr1'),
            'මිය.' => $ci->lang->line('mrs1'),
            'මෙනෙවිය.' => $ci->lang->line('miss')
        );
    }
}
...
if (!function_exists('get_nationality')) {
    ...
    function get_nationality()
    {
        $ci = &get_instance();
        return array(
            'Sri Lankan' => $ci->lang->line('sri_lankan'),
...
if (!function_exists('get_piriven_type')) {

    function get_piriven_type()
    {
        $ci = &get_instance();
        return array(
            'Mulika' => $ci->lang->line('mulika'),
            'Maha' => $ci->lang->line('maha'),
            'Vidyayathana' => $ci->lang->line('vidyayathana'),
            'Seelamatha' => $ci->lang->line('seelamatha'),
            'Dvibhasha' => $ci->lang->line('dvibhasha'),
        );
    }
}

if (!function_exists('get_en_type')) {

    function get_en_type()
    {
        $ci = &get_instance();
        return array(
            'Venarable' => $ci->lang->line('venerable'),
            'Seelamatha' => $ci->lang->line('seelamatha'),
            'Mr' => $ci->lang->line('mr'),
            'Mrs' => $ci->lang->line('mrs'),
            'Miss.' => $ci->lang->line('miss1')
...
if (!function_exists('get_exam')) {
    ...
    function get_exam()
    {
        $ci = &get_instance();
        return array(
            'G.C.E Advanced Level' => $ci->lang->line('exam1'),
            'Prachina Panditha Intermediate Examination' => $ci->lang->line('exam2'),
            'Prachina Panditha Final Examination' => $ci->lang->line('exam3'),
            'Vidyodaya Final Examination' => $ci->lang->line('exam4'),
            'Vidyalankara Final Examination' => $ci->lang->line('exam5'),
            'University First Examination' => $ci->lang->line('exam6'),
            'University Final Examination' => $ci->lang->line('exam7'),
            'Venamulla' => $ci->lang->line('exam8'),
            'Other' => $ci->lang->line('other')
...
if (!function_exists('get_recommendation')) {

    function get_recommendation()
    {
        $ci = &get_instance();
        return array(
            'recommended' => $ci->lang->line('recommended'),
            'not recommended' => $ci->lang->line('not_recommended')
        );
    }
}

if (!function_exists('get_ture_not')) {

    function get_ture_not()
    {
        $ci = &get_instance();
        return array(
            'are true and accurate' => $ci->lang->line('true'),
            'are not true and accurate' => $ci->lang->line('nottrue')
        );
    }
}

if (!function_exists('get_approval')) {

    function get_approval()
    {
        $ci = &get_instance();
        return array(
            'Approved' => $ci->lang->line('appok'),
            'Not Approved' => $ci->lang->line('appnot')
        );
    }
}

if (!function_exists('get_submition')) {

    function get_submition()
    {
        $ci = &get_instance();
        return array(
            'submitted' => $ci->lang->line('submitted'),
            'not submitted' => $ci->lang->line('not_submitted')
        );
    }
}

if (!function_exists('get_given_not')) {

    function get_given_not()
    {
        $ci = &get_instance();
        return array(
            'Given' => $ci->lang->line('given'),
            'Not given' => $ci->lang->line('not_given')
        );
    }
}
...
if (!function_exists('get_inbox_message_postoffice')) {

    function get_inbox_message_postoffice()
    {
        $ci = &get_instance();
        $ci->db->select('MR.*, M.*');
        $ci->db->from('postoffice_relationships AS MR');
        $ci->db->join('postoffice AS M', 'M.id = MR.message_id', 'left');
        $ci->db->where('MR.status', 1);
        $ci->db->where('MR.is_read', 0);
        $ci->db->where('MR.owner_id', logged_in_user_id());
        $ci->db->where('MR.receiver_id', logged_in_user_id());
