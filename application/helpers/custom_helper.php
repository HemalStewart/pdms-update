...
if (!function_exists('logged_in_user_name')) {
    function logged_in_user_name() {
        ...
        // Assuming get_user_name_by_id is already in your helper
        $user_name_obj = get_user_name_by_id($user_id); 
        return $user_name_obj ? $user_name_obj->name : 'N/A';
    }
}
...
function get_user_message_counts() {
    ...
    // --- INBOX COUNT: This query now includes memos by removing the 'is_memo = 0' condition.
    ...
    // --- SENT COUNT: This query now includes memos by removing the 'is_memo = 0' condition.
    ...
    // --- DRAFT COUNT (No change needed here) ---
    $CI->db->select('id');
    ...
    // --- TRASH COUNT (No change needed here) ---
    $CI->db->select('id');
    $CI->db->from('postoffice_relationships');
    $CI->db->where('owner_id', $user_id);
