                ...
                <div class="school-name">
                    <?php if($this->session->userdata('role_id') != PROVINCIAL){ 
                        ...
                        <?php   if ($this->session->userdata('department_id') == 2) {
                             echo $this->global_setting->brand_title . "  -Department User" ;
                        } else{echo $this->session->userdata('school_name');} ?>
                    <?php }
                    else{
                        echo $this->global_setting->brand_title ? $this->global_setting->brand_title : SMS;
                        
                    }}else
                        {
                           echo $this->lang->line('manage_subadmin');
                        }
                        }else
                        {
                           echo $this->lang->line('manage_zonaluser');
                        }
                    }else{ ?>
                         <?php echo $this->session->userdata('provincial_name'); ?>
                    <?php } ?>
                </div>
            </div>
            <div class="col-md-4">
                <ul class="nav navbar-nav <?php echo $this->enable_rtl ? 'navbar-left' : 'navbar-right'; ?>">
                    <li class="">
                        <a href="javascript:;" class="user-profile dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <?php
                                ...
                                elseif($role_id == PROVINCIAL){ $path = 'prov'; }
                                 elseif($role_id == SUBJECT){ $path = 'subjects'; }
                                else{ $path = 'employee'; }
                            ?>
                            <?php if ($photo != '') { ?>                                        
                    ...
                    <?php } ?> 
					 ...
					 <?php $postoffice = get_inbox_message_postoffice(); ?>
                    <?php if(isset($postoffice) && !empty($postoffice)){ ?>
                    <li role="presentation" class="dropdown">
                        <a href="javascript:;" class="dropdown-toggle info-number" data-toggle="dropdown" aria-expanded="false">
                            <i class="fa fa-envelope-o"></i>
                            <span class="badge bg-red"><?php echo count($postoffice); ?></span>
                        </a>
                        <ul id="menu1" class="dropdown-menu list-unstyled msg_list" role="menu">
                            
                           <?php foreach($postoffice as $objx){ ?> 
                            <li>
                                <?php $user = get_user_by_id($objx->sender_id); ?>
                                <a href="<?php echo site_url('postoffice/view/'.$objx->id); ?>">
                                    <span class="image"><img src="<?php echo IMG_URL; ?>default-user.png" alt="Profile Image" /></span>
                                    <span>
                                        <span><?php echo @$user->name; ?></span>
                                        <span class="time"><?php echo get_nice_time($objx->created_at); ?></span>
                                    </span>
                                    <span class="message">
                                        <?php echo $objx->subject; ?>
                                    </span>
                                </a>
                            </li>                    
                            <?php } ?>
                                ...
                                <div class="item form-group">        
                                    <select  class="form-control col-md-7 col-xs-12" name="search_school_id" id="search_school_id" required="required">
                                        ...
                                        <?php foreach ($this->schools as $obj) { ?>
                                            <option value="<?php echo $obj->id; ?>" ><?php echo $obj->school_name; ?>  - ( <?php echo $obj->cencus_number; ?> )</option>
                                        <?php } ?>
                                    </select>       
                                </div>
                            </div>
