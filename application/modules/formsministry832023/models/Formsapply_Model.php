...
class Formsapply_Model extends MY_Model {
    ...
    public function get_applicationapproved_list($id = null){
     ...
     /*  $status = 6;
       // $str_area = explode(',', $zonal_id);
        ...
        $this->db->from('new_teacher_app_01 AS TP');
        $this->db->join('schools AS SC', 'SC.id = TP.school_id', 'left');
        $this->db->where('TP.cur_status', $status);
	...
	public function get_applicationpending_list($id = null){
...
/*     $this->db->where('TP.status');
       $this->db->where('TP.approved_status', 1);
        $this->db->where('TP.confirmstatus', 2);
        $this->db->where('TP.reject_status !=', 2);*/
