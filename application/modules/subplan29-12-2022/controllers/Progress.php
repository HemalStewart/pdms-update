...
//            $this->data['add_vehicles'] = $this->route->get_vehicle_list($condition['school_id'], '');
        ...
        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('subplanprogress') . ' | ' . SMS);
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->route->insert('subplan_monthlydetails', $data);
                if ($insert_id) {

                    $this->_save_prozo($insert_id);
                    success($this->lang->line('insert_success'));
                    redirect('subplan/progress/index/' . $data['school_id']);
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('subplan/progress/add' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('insert_failed'));
    ...
    public function edit($id = null) {
        ...
        if (!is_numeric($id)) {
            ...
            redirect('subplan/progress/index');
        }
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_prozo_data();
                $updated = $this->route->update('subplan_monthlydetails', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {
                    ...
                    redirect('subplan/progress/edit/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['route'] = $this->route->get_single('subplan_monthlydetails', array('id' => $this->input->post('id')));
            }
        }

        if ($id) {
            $this->data['route'] = $this->route->get_single('subplan_monthlydetails', array('id' => $id));
        ...
        $this->data['routes'] = $this->route->get_progress_list($user_id);
        $this->data['route_stops'] = $this->route->get_list('subplan_programme', array('status' => 1, 'prozo_id' => $id), '', '', '', '', 'id', 'ASC');
...
//        $items[] = 'note';
    ...
    public function delete($id = null) {
        ...
        $route = $this->route->get_single('subplan_monthlydetails', array('id' => $id));

        if ($this->route->delete('subplan_monthlydetails', array('id' => $id))) {


            // delete route stop
            $this->route->delete('subplan_programme', array('route_id' => $id));
        ...
        redirect('subplan/progress/index/' . $route->user_id);
    }
    ...
    private function _save_prozo($prozo_id) {
        ...
        foreach ($this->input->post('proposed_date') as $key => $value) {
            ...
            if ($value) {
                ...
                if ($tea_id) {
                    $exist = $this->route->get_single('subplan_programme', array('prozo_id' => $prozo_id, 'id' => $tea_id));
                }
                ...
                if ($this->input->post('id') && $exist) {
                    ...
                    $data['modified_at'] = date('Y-m-d H:i:s');
                    $data['modified_by'] = logged_in_user_id();
                    $this->route->update('subplan_programme', $data, array('id' => $exist->id));
                } else {
                    ...
                    $data['created_at'] = date('Y-m-d H:i:s');
                    $data['created_by'] = logged_in_user_id();
                    $this->route->insert('subplan_programme', $data);
                }
            }
        }
    }
    ...
    public function remove_stop() {
        ...
        echo $this->route->delete('subplan_programme', array('id' => $stop_id));
    }
