...
class Formsapply extends My_Controller {
    ...
    public function __construct() {
		...
		$this->data['roles'] = $this->forms->get_list('roles', array('status' => 1, 'is_super_admin'=>0), '','', '', 'id', 'ASC');
        $this->data['classes'] = $this->forms->get_list('classes', array('status' => 1), '','', '', 'id', 'ASC');
		
		 $tappforms = $this->session->userdata('tappforms');
         $listsubjects = $this->forms->listsubjects($tappforms);
...
//        }
    ...
    public function get_single_teacher_application_form() {
		...
		$this->data['forms1'] = $this->forms->get_new_teacher_app_01_i($teacherapplicationform_id);		
		$this->data['new_teacher_app_01_working'] = $this->forms->get_new_teacher_app_01_working($teacherapplicationform_id);
		$this->data['forms2'] = $this->forms->get_new_teacher_app_02($teacherapplicationform_id);
		$this->data['new_teacher_app_02_class_sub'] = $this->forms->get_new_teacher_app_01_subclass($teacherapplicationform_id);
		$this->data['forms3'] = $this->forms->get_new_teacher_app_03($teacherapplicationform_id);
    ...
    public function _get_teacherapp_data1_i($teacher_app_id) {
        ...
        $data['created_by'] = logged_in_user_id();
		 ...
		 if ($_FILES['applicant_sig1']['name']) {
	      ...
	      if ($this->input->post('schlform_id')) {
					
					$schoolid = $this->input->post('schlform_id');

//                    $data['modified_at'] = date('Y-m-d H:i:s');
//                    $data['modified_by'] = logged_in_user_id();
                    $this->forms->update('new_teacher_app_01_i', $data, array('id' => $schoolid));
                } else {
...
//                    $data['modified_at'] = date('Y-m-d H:i:s');
//                    $data['modified_by'] = logged_in_user_id();
                    $this->forms->update('new_teacher_app_02', $data, array('id' => $schoolid));
                } else {
...
//                    $data['created_by'] = logged_in_user_id();
	...
	/*	teb part 4 details*/
        ...
        $data = elements($items, $_POST);
          ...
          $data['received_datexvi'] = date('Y-m-d', strtotime($this->input->post('received_datexvi')));
	  ...
	  if ($_FILES['accepted_sig1']['name']) {
           $data['accepted_sig1'] = $this->_upload_accepted_sig1();
       }
...
//                    $data['modified_at'] = date('Y-m-d H:i:s');
//                    $data['modified_by'] = logged_in_user_id();
                    $this->forms->update('new_teacher_app_03', $data, array('id' => $schoolid));
                } else {
...
//                    $data['created_by'] = logged_in_user_id();
	...
	private function _upload_accepted_sig1() {

        $prev_accepted_sig1 = $this->input->post('prev_accepted_sig1');
        $accepted_sig1 = $_FILES['accepted_sig1']['name'];
        $accepted_sig1_type = $_FILES['accepted_sig1']['type'];
        $return_accepted_sig1 = '';
        if ($accepted_sig1 != "") {
            if ($accepted_sig1_type == 'image/jpeg' || $accepted_sig1_type == 'image/pjpeg' ||
                    $accepted_sig1_type == 'image/jpg' || $accepted_sig1_type == 'image/png' ||
                    $accepted_sig1_type == 'image/x-png' || $accepted_sig1_type == 'image/gif') {

                $destination = 'assets/uploads/forms/teacher_application/';

                $file_type = explode(".", $accepted_sig1);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $accepted_sig1_path = 'photo-' . time() . '-sms.' . $extension;

                move_uploaded_file($_FILES['accepted_sig1']['tmp_name'], $destination . $accepted_sig1_path);

                // need to unlink previous mother_photo
                if ($prev_accepted_sig1 != "") {
                    if (file_exists($destination . $prev_accepted_sig1)) {
                        @unlink($destination . $prev_accepted_sig1);
                    }
                }

                $return_accepted_sig1 = $accepted_sig1_path;
            }
        } else {
            $return_accepted_sig1 = $prev_accepted_sig1;
        }

        return $return_accepted_sig1;
    }
...
//PRINT--------------
    public function teacher_application_print($id) {
		...
		$this->data['new_teacher_app_01_working'] = $this->forms->get_new_teacher_app_01_working($id);
		$this->data['forms2'] = $this->forms->get_new_teacher_app_02($id);
		$this->data['new_teacher_app_02_class_sub'] = $this->forms->get_new_teacher_app_01_subclass($id);
 ...
 //echo "<script type='text/javascript'>alert('$message');</script>";
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
					...
					$this->_get_teacherapp_data1_i($this->input->post('schlform_id'));
					$this->_save_teacherapp_data1_i_working($this->input->post('schlform_id'));
					$this->_get_teacherapp_data2($this->input->post('schlform_id'));
					$this->_get_teacherapp_data3($this->input->post('schlform_id'));
					
                    success($this->lang->line('update_success'));
                    redirect('forms/formsapply/teacherapplicationform' . $data['schlform_id']);
                } else {
                    error($this->lang->line('update_failed'));
