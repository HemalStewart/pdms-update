...
class Dispatchall extends MY_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('Dispatch_Modelall', 'dispatchall', true);   
...
//<!---************************************************-->
        ...
        $this->data['dispatches'] = $this->dispatchall->get_dispatch($school_id); 
        //$this->data['filter_school_id'] = $school_id;
        $this->data['schools'] = $this->schools;
        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_postal_dispatch').' | ' . SMS);
        $this->layout->view('dispatchall/index', $this->data);  
    }
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->dispatchall->insert('postal_dispatchesall', $data);
                if ($insert_id) {
                    success($this->lang->line('insert_success'));
                    redirect('frontoffice/dispatchall/index/'.$data['school_id']);
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('frontoffice/dispatchall/add');
                }
            } else {
                error($this->lang->line('insert_failed'));
                $this->data = $_POST;
        ...
        $this->data['dispatches'] = $this->dispatchall->get_dispatch();        
        $this->data['schools'] = $this->schools;
    ...
    * ********************************************************** */
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_dispatch_data();
                $updated = $this->dispatchall->update('postal_dispatchesall', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {                   
                    ...
                    redirect('frontoffice/dispatchall/edit/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['dispatch'] = $this->dispatchall->get_single('postal_dispatchesall', array('id' => $this->input->post('id')));
            }
        } else {
            if ($id) {
                $this->data['dispatch'] = $this->dispatchall->get_single('postal_dispatchesall', array('id' => $id));
// echo "<script>alert('" . $id . "');</script>";
                if (!$this->data['dispatch']) {
                     redirect('frontoffice/dispatchall/index');
                }
            }
        }
 //echo "<script>alert('Done');</script>";
      ...
      $this->data['schools'] = $this->schools;
        $this->data['dispatches'] = $this->dispatchall->get_dispatch($this->data['school_id']); 
    ...
    public function get_single_dispatch(){
       ...
       $this->data['dispatch'] = $this->dispatchall->get_single_dispatch($dispatch_id);
       echo $this->load->view('dispatchall/get-single-dispatch', $this->data);
    }
    ...
    * ********************************************************** */
    private function _upload_attachment() {
        ...
        if ($attachment != "") {
                ...
                $attachment_path = 'dispatchall-' . time() . '-doc.' . $extension;
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $dispatch = $this->dispatchall->get_single('postal_dispatchesall', array('id' => $id));        
        if ($this->dispatchall->delete('postal_dispatchesall', array('id' => $id))) { 
            ...
            redirect('frontoffice/dispatchall/index/'.$dispatch->school_id);
        } else {
