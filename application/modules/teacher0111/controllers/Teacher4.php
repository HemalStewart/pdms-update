...
//        $this->data['districtlist'] = $this->teacher->get_list('district', $province_id, '', '', '', 'id', 'ASC');
    ...
    private function _get_posted_teacher_data() {
        ...
        $data['pension_date'] = date('Y-m-d', strtotime($this->input->post('pention_date')));
    ...
    public function indexprovince($school_id = null) {
        ...
        $this->data['teachers'] = $this->teacher->get_teacher_provincelist($provincial_id);
        $this->data['roles'] = $this->teacher->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
    ...
    public function addtype() {
        ...
        //    $this->data['types'] = $this->teacher->get_type(); 
        $this->data['schools'] = $this->schools;
    ...
    public function type() {
        if ($this->input->post('id') == '') {
            $type = $this->teacher->duplicate_typecheck($this->input->post('school_id'), $this->input->post('type'));
            if ($type) {
            ...
            $type = $this->teacher->duplicate_typecheck($this->input->post('school_id'), $this->input->post('type'), $this->input->post('id'));
            if ($type) {
                $this->form_validation->set_message('type', $this->lang->line('already_exist'));
                return FALSE;
            } else {
