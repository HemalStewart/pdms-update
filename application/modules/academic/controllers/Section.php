...
//        $this->data['sections'] = $this->section->get_section_list($class_id, $school_id); 
        $this->data['sections'] = $this->section->get_section_list($province_id, $district_id, $zonal_id, $edu_id, $schooln_id, $nclass_id);
        ...
        if ($this->session->userdata('role_id') == PROVINCIAL) {
            ...
            $this->data['district'] = $this->section->getdistrictData($province_id);
        }

        if ($this->session->userdata('role_id') == ZONAL) {
            $zonal_id = $this->session->userdata('zonal_id');
            $this->data['education'] = $this->section->getzonalData($zonal_id);
        }


        $pirtype = $this->session->userdata('pirtype');
        $listclass = $this->section->listclassid($pirtype);
    ...
    private function _get_posted_section_data() {
        ...
        // $data['school_classesid'] = $this->input->post('class_id');
