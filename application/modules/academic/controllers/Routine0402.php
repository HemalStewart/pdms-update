    ...
    public function index($class_id = null) {
      ...
      //  $this->data['class_id'] = $class_id;
        ...
        if($_POST){
            ...
            // $school_id = $this->input->post('school_id');
          //  $nclass_id = $this->input->post('nclass_id');
            
        }else{
           $class = $this->routine->getsingleclassData($class_id);  
     ...
     //   $this->data['filter_nclass_id'] = $nclass_id;
        $this->data['filter_class_id'] = $class_id;
        
      //  $this->data['sections'] = $this->routine->get_section_list($class_id, $school_id); 
        $this->data['sections'] = $this->routine->get_section_list($province_id, $district_id, $zonal_id, $edu_id, $school_id, $class_id); 
        
      //  $this->data['single_class'] = $this->routine->get_single('classes', array('id' => $class_id));      
       $this->data['single_class'] = $this->routine->getsingleclassData($class_id);
        
        $condition = array();
        
        $condition['status'] = 1; 
         $pirtype = $this->session->userdata('pirtype');
        $listclass = $this->routine->getclassData($pirtype);
        ...
        if($this->session->userdata('role_id') == ADMIN || $this->session->userdata('role_id') == GUARDIAN || $this->session->userdata('role_id') == STUDENT || $this->session->userdata('role_id') == TEACHER || $this->session->userdata('role_id') == ACCOUNTANT || $this->session->userdata('role_id') == LIBRARIN || $this->session->userdata('role_id') == RECEPTIONIST || $this->session->userdata('role_id') == STAFF){ 
          ...
          if ($this->session->userdata('role_id') == PROVINCIAL) {
            ...
            $this->data['district'] = $this->routine->getdistrictData($province_id);
        }

        if ($this->session->userdata('role_id') == ZONAL) {
            $zonal_id = $this->session->userdata('zonal_id');
            $this->data['education'] = $this->routine->getzonalData($zonal_id);
        }
