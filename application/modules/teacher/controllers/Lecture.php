...
class Lecture extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null) {
        ...
        if ($this->session->userdata('role_id') == PROVINCIAL) {
            ...
            $this->data['district'] = $this->lecture->getdistrictData($province_id);
        }

        if ($this->session->userdata('role_id') == ZONAL) {
            $zonal_id = $this->session->userdata('zonal_id');
            $this->data['education'] = $this->lecture->getzonalData($zonal_id);
        }
        ...
        $listclass = $this->lecture->getclassData($pirtype);
    ...
    * ********************************************************** */
    private function _get_posted_lecture_data() {
        ...
        $subject = $this->lecture->get_single('subjects', array('Submainid' => $data['subject_id']));
