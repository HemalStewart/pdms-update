        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    redirect('formsministry/formsapply/teacherapplicationformedit/' . $this->input->post('schlform_id'));
                }
            } else {
                error($this->lang->line('update_failed'));
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    // $this->_save_prozo($this->input->post('id'));
                    success($this->lang->line('reject'));
                     redirect('formsministry/formsapply/teacherapplicationform' . $data['school_id']);
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('formsministry/formsapply/reject/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['route'] = $this->forms->get_single('new_teacher_app_01', array('id' => $this->input->post('id')));
...
//        $this->data['route_stops'] = $this->route->get_list('prom_programme', array('status' => 1, 'prozo_id' => $id), '', '', '', '', 'id', 'ASC');
        ...
        $this->data['edit'] = TRUE;
        $this->layout->title($this->lang->line('edit') . ' | ' . SMS);
        $this->layout->view('teacherapplication/indexreject', $this->data);
    }
...
//        $items[] = 'route_end';
	 ...
	 public function approvereject($id = null) {
        ...
        if ($_POST) {
            $this->_prepare_confirm_validation();
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_approvereject_data();
                $updated = $this->forms->update('new_teacher_app_01', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {
                    ...
                    error($this->lang->line('update_failed'));
                    redirect('formsministry/formsapply/approvereject/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['route'] = $this->forms->get_single('new_teacher_app_01', array('id' => $this->input->post('id')));
...
//        $this->data['route_stops'] = $this->route->get_list('prom_programme', array('status' => 1, 'prozo_id' => $id), '', '', '', '', 'id', 'ASC');
        ...
        $this->data['edit'] = TRUE;
        $this->layout->title($this->lang->line('edit') . ' | ' . SMS);
        $this->layout->view('teacherapplication/indexapprovereject', $this->data);
    }
	
	  private function _get_posted_approvereject_data() {
...
//        $items[] = 'route_start';
//        $items[] = 'route_end';
       $items[] = 'approverejectnote';

        $data = elements($items, $_POST);

        $data['cur_status'] = 6;
        $data['approved_status'] = 1;
        $data['approvereject_status'] = 2;
        $data['approvereject_at'] = date('Y-m-d H:i:s');
        $data['approvereject_by'] = logged_in_user_id();
 ...
 //echo "<script type='text/javascript'>alert('$message');</script>";
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    success($this->lang->line('update_success'));
                    redirect('formsministry/formsapply/teacherapplicationform' . $data['id']);
                } else {
                    error($this->lang->line('update_failed'));
        ...
        $this->data['edit'] = TRUE;
        $this->layout->title($this->lang->line('edit') . ' | ' . SMS);
        $this->layout->view('teacherapplication/index_teacher_application_edit', $this->data);
    }
