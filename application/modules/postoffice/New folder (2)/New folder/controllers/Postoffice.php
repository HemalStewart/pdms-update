...
class Postoffice extends MY_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('Postoffice_Model', 'postoffice', true);
        
        $this->load->model('Ajax_Model', 'ajax', true);
        $listprovincial = $this->ajax->listprovincial();
        $this->data['listprovincial'] = $listprovincial;

        $this->data['sents'] = $this->postoffice->get_message_list($type = 'sent');
        $this->data['drafts'] = $this->postoffice->get_message_list($type = 'draft');
        $this->data['trashs'] = $this->postoffice->get_message_list($type = 'trash');
        $this->data['inboxss'] = $this->postoffice->get_message_list($type = 'inbox');
        $this->data['new'] = $this->postoffice->get_message_list($type = 'new');
    ...
    public function compose($id = null) {
       ...
       /* check_permission(ADD);*/
        ...
        if ($_POST) {
            ...
            $school = $this->postoffice->get_school_by_id($data['school_id']);
            ...
            if ($this->input->post('postoffice_id')) {
                $data['modified_at'] = date('Y-m-d H:i:s');
                $data['modified_by'] = logged_in_user_id();
                $this->postoffice->update('postoffice', $data, array('id' => $this->input->post('postoffice_id')));
            } else {
                $data['status'] = 1;
                $data['created_at'] = date('Y-m-d H:i:s');
                $data['created_by'] = logged_in_user_id();
                $insert_id = $this->postoffice->insert('postoffice', $data);
            }
            ...
            if ($this->input->post('postoffice_id')) {
                $relation_data['message_id'] = $this->input->post('postoffice_id');
            ...
            if (isset($_POST['draft'])) { // if draft
                ...
                if ($this->input->post('postoffice_id')) {
                    $condition = array('message_id' => $this->input->post('postoffice_id'), 'owner_id' => logged_in_user_id());
                    $this->postoffice->update('postoffice_relationships', $relation_data, $condition);
                } else {
                    $relation_data['owner_id'] = logged_in_user_id();
                    $relation_data['is_draft'] = 1;
                    $this->postoffice->insert('postoffice_relationships', $relation_data);
                }
        ...
        if ($id) {
            $this->data['postoffice'] = $this->postoffice->get_single_message($id);
        }
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            ...
            $this->data['classes'] = $this->postoffice->get_list('classes', $condition, '', '', '', 'id', 'ASC');
        } 

        $this->data['roles'] = $this->postoffice->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function view($id = null) {
       ...
       /* check_permission(VIEW);*/
        ...
        if ($id) {
            $this->data['postoffice'] = $this->postoffice->get_single_message($id);            
            $this->postoffice->update('postoffice_relationships', array('is_read' => 1), array('message_id' => $id, 'owner_id'=> logged_in_user_id()));
            
            $this->data['replies'] = $this->postoffice->get_list('replies', array('message_id' => $id), '', '', '', 'id', 'ASC');
    ...
    public function reply($id = null) {
       ...
       /* check_permission(ADD);*/
        ...
        if ($_POST) {
            ...
            $data['created_at'] = date('Y-m-d H:i:s');
            $data['created_by'] = logged_in_user_id();
            if ($this->postoffice->insert('replies', $data)) {
                success($this->lang->line('insert_success'));
            } else {
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $exist = $this->postoffice->get_list('postoffice_relationships', array('status' => 1, 'message_id' => $id, 'owner_id !=' => logged_in_user_id()));

        if (!empty($exist)) {
            $data = array('status' => 0);
            $condition = array('message_id' => $id, 'owner_id' => logged_in_user_id());
            $this->postoffice->update('postoffice_relationships', $data, $condition);
            success($this->lang->line('delete_success'));
        } else {
            $this->postoffice->delete('postoffice_relationships', array('message_id' => $id));
            $this->postoffice->delete('postoffice', array('id' => $id));
            $this->postoffice->delete('replies', array('message_id' => $id));
            success($this->lang->line('delete_success'));
        }
