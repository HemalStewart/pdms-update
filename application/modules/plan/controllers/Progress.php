        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    ...
                    redirect('plan/progress/add' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('insert_failed'));
...
//        $items[] = 'is_view_on_web';
      ...
      //  $implodezonal_id = implode(',', $this->input->post('zonal_id'));
        ...
        if ($this->input->post('id')) {
            ...
            if ($role_id == 19) {
                if($this->input->post('zonal_id')){
                $editimplodezonal_id = implode(',', $this->input->post('zonal_id'));
                if ($editimplodezonal_id) {

                    $data['zonal_id'] = $editimplodezonal_id;
                }
            }
...
//                $district_id = $this->input->post('district_id');
                $provincial_id = $this->input->post('provincialidval');
                $implodezonal_id = implode(',', $this->input->post('zonal_id'));
                $data['provincial_name'] = $provincial_name;
                $data['provincial_id'] = $provincial_id;
...
//                $data['cost_institution'] = @$_POST['cost_institution'][$key];
    ...
    public function getprogressdate() {
        $products = $this->route->getprogressdate();
        echo json_encode($products);
    }
