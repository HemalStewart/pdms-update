...
class Superadmin extends MY_Controller {
    ...
    function __construct() {
        ...
        $listsub = $this->ajax->get_listsub();
        $this->data['listsub'] = $listsub;

        $this->data['listsub'] = $this->superadmin->get_listsub();
    ...
    public function indexProvincial() {
        ...
        $this->data['provinceadmins'] = $this->superadmin->get_provinceadmin_list();
        $this->data['roles'] = $this->superadmin->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');

        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_super_admin') . ' | ' . SMS);
        $this->layout->view('super_admin/provincial', $this->data);
    }
    ...
    public function addprovince() {
        ...
        if ($_POST) {
            $this->_prepare_superadmin_validation();
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_provincialadmin_data();

                $insert_id = $this->superadmin->insert('provincial_dir', $data);
                if ($insert_id) {

                    create_log('Has been created a Provincial admin : ' . $data['name']);

                    success($this->lang->line('insert_success'));
                    redirect('administrator/superadmin/indexProvincial');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('administrator/superadmin/addprovince');
                }
            } else {
                error($this->lang->line('insert_failed'));
                $this->data['post'] = $_POST;
    ...
    public function editprovince($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_provincialadmin_data();
                $updated = $this->superadmin->update('provincial_dir', $data, array('id' => $this->input->post('id')));

                if ($updated) {

                    create_log('Has been updated a Provincial admin : ' . $data['name']);

                    success($this->lang->line('update_success'));
                    redirect('administrator/superadmin/indexProvincial');
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('administrator/superadmin/editprovince/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['superadmin'] = $this->superadmin->get_single_provadmin($this->input->post('id'));
            }
        } else {
            if ($id) {
                $this->data['superadmin'] = $this->superadmin->get_single_provadmin($id);
    ...
    public function deleteprovince($id = null) {
        ...
        $superadmin = $this->superadmin->get_single('provincial_dir', array('id' => $id));
        if (!empty($superadmin)) {

            create_log('Has been deleted a Provincial admin : ' . $superadmin->name);

            // delete superadmin data
            $this->superadmin->delete('provincial_dir', array('id' => $id));
            // delete superadmin login data
            $this->superadmin->delete('users', array('id' => $superadmin->user_id));
            ...
            if (file_exists($destination . '/prov-photo/' . $superadmin->photo)) {
                @unlink($destination . '/prov-photo/' . $superadmin->photo);
            }
    ...
    public function get_single_provincial_admin() {

        $super_admin_id = $this->input->post('super_admin_id');

        $this->data['superadmin'] = $this->superadmin->get_single_provadmin($super_admin_id);
        echo $this->load->view('super_admin/get-single-super-admin', $this->data);
    }

    private function _get_posted_provincialadmin_data() {
...
//                  if ($this->input->post('id')) {
//            $provincial = $this->input->post('hiddenprovincial');
//            $editprovincial = $this->input->post('provincial_id');
////
//            if ($provincial != $editprovincial) {
//
//                $data['provincial_id'] = $this->input->post('edit_provincialid');
//            }
//        } 

        $data['dob'] = date('Y-m-d', strtotime($this->input->post('dob')));

//        $data['provincial_id'] = 2;
        ...
        if ($_FILES['photo']['name']) {
            $data['photo'] = $this->_upload_provphoto();
        }
        if ($_FILES['resume']['name']) {
            $data['resume'] = $this->_upload_resume();
        }
    ...
    private function _upload_provphoto() {
        ...
        if ($photo != "") {
            if ($photo_type == 'image/jpeg' || $photo_type == 'image/pjpeg' ||
                ...
                // super admin photo folder is same as employee
                $destination = 'assets/uploads/provincial-photo/';
    ...
    public function indexZonal() {

        //   check_permission(VIEW);

        $this->data['provinceadmins'] = $this->superadmin->get_zonaladmin_list();
        $this->data['roles'] = $this->superadmin->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');

        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_super_admin') . ' | ' . SMS);
        $this->layout->view('super_admin/zonal', $this->data);
    }
    ...
    public function addzonal() {
        ...
        if ($_POST) {
            $this->_prepare_superadmin_validation();
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_zonaladmin_data();

                $insert_id = $this->superadmin->insert('zonal_dir', $data);
                if ($insert_id) {
                    //$this->_save_zonal($insert_id);
                    create_log('Has been created a Zonal admin : ' . $data['name']);

                    success($this->lang->line('insert_success'));
                    redirect('administrator/superadmin/indexZonal');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('administrator/superadmin/addzonal');
                }
            } else {
                error($this->lang->line('insert_failed'));
                $this->data['post'] = $_POST;
    ...
    public function editzonal($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_zonaladmin_data();
                $updated = $this->superadmin->update('zonal_dir', $data, array('id' => $this->input->post('id')));

                if ($updated) {

                    create_log('Has been updated a Zonal admin : ' . $data['name']);

                    success($this->lang->line('update_success'));
                    redirect('administrator/superadmin/indexZonal');
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('administrator/superadmin/editzonal/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['superadmin'] = $this->superadmin->get_single_zonaladmin($this->input->post('id'));
            }
        } else {
            if ($id) {

                $this->data['superadmin'] = $this->superadmin->get_single_zonaladmin($id);
    ...
    public function deletezonal($id = null) {
        ...
        $superadmin = $this->superadmin->get_single('zonal_dir', array('id' => $id));
        if (!empty($superadmin)) {

            create_log('Has been deleted a Zonal admin : ' . $superadmin->name);

            // delete superadmin data
            $this->superadmin->delete('zonal_dir', array('id' => $id));
            // delete superadmin login data
            $this->superadmin->delete('users', array('id' => $superadmin->user_id));
    ...
    public function get_single_zonal_admin() {

        $super_admin_id = $this->input->post('super_admin_id');

        $this->data['superadmin'] = $this->superadmin->get_single_zonaladmin($super_admin_id);
        echo $this->load->view('super_admin/get_single_zonal_admin', $this->data);
    }

    private function _get_posted_zonaladmin_data() {
        ...
        $permissionMinistry = serialize($this->input->post('zonal_id'));
//
//            if ($provincial != $editprovincial) {
//
//                $items[] = 'provincial_id';
...
//        }
        ...
        $data = elements($items, $_POST);
        $data['zonal_id'] = $implodezonal_id;
        $data['zonalarray_id'] = $permissionMinistry;
        ...
        if ($_FILES['photo']['name']) {
            $data['photo'] = $this->_upload_zonalphoto();
        }
        if ($_FILES['resume']['name']) {
            $data['resume'] = $this->_upload_resume();
        }
    ...
    private function _upload_zonalphoto() {
        ...
        if ($photo != "") {
            if ($photo_type == 'image/jpeg' || $photo_type == 'image/pjpeg' ||
                ...
                // super admin photo folder is same as employee
                $destination = 'assets/uploads/zonal-photo/';
    ...
    public function indexsubject() {
        ...
        $this->data['subjectadmins'] = $this->superadmin->get_subjectadmin_list();
        $this->data['roles'] = $this->superadmin->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');

        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_super_admin') . ' | ' . SMS);
        $this->layout->view('super_admin/subjectuser', $this->data);
    }
    ...
    public function addsubject() {
        ...
        if ($_POST) {
            $this->_prepare_superadmin_validation();
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_subjectadmin_data();

                $insert_id = $this->superadmin->insert('subject_dir', $data);
                if ($insert_id) {

                    create_log('Has been created a Subject admin : ' . $data['name']);

                    success($this->lang->line('insert_success'));
                    redirect('administrator/superadmin/indexsubject');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('administrator/superadmin/addsubject');
                }
            } else {
                error($this->lang->line('insert_failed'));
                $this->data['post'] = $_POST;
    ...
    public function editsubject($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_subjectadmin_data();
                $updated = $this->superadmin->update('subject_dir', $data, array('id' => $this->input->post('id')));

                if ($updated) {

                    create_log('Has been updated a Subject admin : ' . $data['name']);

                    success($this->lang->line('update_success'));
                    redirect('administrator/superadmin/indexsubject');
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('administrator/superadmin/editsubject/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['superadmin'] = $this->superadmin->get_single_subjectadmin($this->input->post('id'));
            }
        } else {
            if ($id) {
                $this->data['superadmin'] = $this->superadmin->get_single_subjectadmin($id);
        ...
        $this->data['subjectadmins'] = $this->superadmin->get_subjectadmin_list();
        $this->data['roles'] = $this->superadmin->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        $this->data['sublist'] = $this->superadmin->get_list('sub_list', array('status' => 1), '', '', '', 'id', 'ASC');
    ...
    public function deletesubject($id = null) {
        ...
        $superadmin = $this->superadmin->get_single('subject_dir', array('id' => $id));
        if (!empty($superadmin)) {

            create_log('Has been deleted a Subject admin : ' . $superadmin->name);

            // delete superadmin data
            $this->superadmin->delete('subject_dir', array('id' => $id));
            // delete superadmin login data
            $this->superadmin->delete('users', array('id' => $superadmin->user_id));
    ...
    public function get_single_subject_admin() {

        $super_admin_id = $this->input->post('super_admin_id');

        $this->data['superadmin'] = $this->superadmin->get_single_subjectadmin($super_admin_id);
        echo $this->load->view('super_admin/get-single-super-admin', $this->data);
    }

    private function _get_posted_subjectadmin_data() {
...
//        }
        ...
        $implodesubject_id = implode(',', $this->input->post('subject_id'));
        $permissionsubject_id = serialize($this->input->post('subject_id'));
        $data['subject_id'] = $implodesubject_id;
        $data['subjectarray_id'] = $permissionsubject_id;
        ...
        if ($_FILES['photo']['name']) {
            $data['photo'] = $this->_upload_subjectphoto();
        }
        if ($_FILES['resume']['name']) {
            $data['resume'] = $this->_upload_resume();
        }
    ...
    private function _upload_subjectphoto() {
        ...
        if ($photo != "") {
            if ($photo_type == 'image/jpeg' || $photo_type == 'image/pjpeg' ||
                ...
                // super admin photo folder is same as employee
                $destination = 'assets/uploads/subject-photo/';
    ...
    public function indexDepartment() {
        ...
        $this->data['departmentadmins'] = $this->superadmin->get_departmentadmin_list();
        $this->data['roles'] = $this->superadmin->get_list('roles', array('status' => 1, 'showtype' => 1), '', '', '', 'id', 'ASC');

        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_department_admin') . ' | ' . SMS);
        $this->layout->view('super_admin/department', $this->data);
    }
    ...
    public function addDepartment() {
        ...
        if ($_POST) {
            $this->_prepare_superadmin_validation();
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_Departmentadmin_data();

                $insert_id = $this->superadmin->insert('department_dir', $data);
                if ($insert_id) {

                    create_log('Has been created a Department admin : ' . $data['name']);

                    success($this->lang->line('insert_success'));
                    redirect('administrator/superadmin/indexDepartment');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('administrator/superadmin/addDepartment');
                }
            } else {
                error($this->lang->line('insert_failed'));
                $this->data['post'] = $_POST;
    ...
    public function editDepartment($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_Departmentadmin_data();
                $updated = $this->superadmin->update('department_dir', $data, array('id' => $this->input->post('id')));

                if ($updated) {

                    create_log('Has been updated a Department admin : ' . $data['name']);

                    success($this->lang->line('update_success'));
                    redirect('administrator/superadmin/indexDepartment');
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('administrator/superadmin/editDepartment/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['superadmin'] = $this->superadmin->get_single_departmentadmin($this->input->post('id'));
            }
        } else {
            if ($id) {
                $this->data['superadmin'] = $this->superadmin->get_single_departmentadmin($id);
    ...
    public function deleteDepartment($id = null) {
        ...
        $superadmin = $this->superadmin->get_single('department_dir', array('id' => $id));
        if (!empty($superadmin)) {

            create_log('Has been deleted a Department admin : ' . $superadmin->name);

            // delete superadmin data
            $this->superadmin->delete('department_dir', array('id' => $id));
            // delete superadmin login data
            $this->superadmin->delete('users', array('id' => $superadmin->user_id));
...
//            }
    ...
    public function get_single_Department_admin() {

        $super_admin_id = $this->input->post('super_admin_id');

        $this->data['superadmin'] = $this->superadmin->get_single_departmentadmin($super_admin_id);
        echo $this->load->view('super_admin/get-single-department-admin', $this->data);
    }

    private function _get_posted_Departmentadmin_data() {
        ...
        $data['subdepartment_id'] = $this->input->post('subdepartment_id');
...
//        $data['provincial_id'] = 2;
        ...
        if ($this->input->post('id')) {
            ...
            $data['modified_at'] = date('Y-m-d H:i:s');
            $data['modified_by'] = logged_in_user_id();
            $this->superadmin->update('users', array('role_id' => $this->input->post('role_id'), 'modified_at' => date('Y-m-d H:i:s')), array('id' => $this->input->post('user_id')));
        } else {
            $data['created_at'] = date('Y-m-d H:i:s');
            $data['created_by'] = logged_in_user_id();
            $data['status'] = 1;
        ...
        if ($_FILES['photo']['name']) {
            $data['photo'] = $this->_upload_Departmentphoto();
        }
        if ($_FILES['resume']['name']) {
            $data['resume'] = $this->_upload_resume();
        }
    ...
    private function _upload_Departmentphoto() {
        ...
        if ($photo != "") {
            if ($photo_type == 'image/jpeg' || $photo_type == 'image/pjpeg' ||
                ...
                // super admin photo folder is same as employee
                $destination = 'assets/uploads/department-photo/';
