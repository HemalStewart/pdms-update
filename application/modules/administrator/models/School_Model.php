...
class School_Model extends MY_Model {
    ...
    {
        $this->db->select('SC.*, P.provincialname, DI.districtname, Z.zonename, ZB.zoneblock,
                           SC.piriven_dev_fund_account_name, SC.piriven_dev_fund_account_number,
                           SC.piriven_dev_fund_bank, SC.piriven_dev_fund_branch,
                           SC.kruthydhikari_account_name, SC.kruthydhikari_account_number,
                           SC.kruthydhikari_bank, SC.kruthydhikari_branch');
    ...
    public function geteditSchoolData($id = null) {
        ...
        if ($id) {
            // Combine all necessary columns into a single select statement
            $this->db->select('schools.*, provincial.provincialname, district.districtname, zone.zonename, zoneblock.zoneblock');
        ...
        // This section is for the main list view, it's correct as is, but could be cleaner.
        // It does not need the new bank detail fields.
...
//
    ...
    // Add the necessary JOINs to link the school with its location names
    ...
    public function get_expenditure_info($school_id) {
        $this->db->select('E.*');
        $this->db->from('expenditure_heads AS E');
    ...
    public function get_employees_info($school_id) {
        $this->db->select('EP.*, D.name AS designation');
        $this->db->from('employees AS EP');
		 $this->db->join('designations AS D', 'D.id = EP.designation_id', 'left');
		
		 $des_names = array('Principal', 'Kruthyadhikari');
         $this->db->where_in('D.name', $des_names);

       $this->db->where('EP.school_id', $school_id);
        $this->db->order_by('EP.id');
        $query = $this->db->get();
        return $query->result();
