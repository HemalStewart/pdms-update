...
class Formsapply extends My_Controller {
    ...
    public function teacherapplicationform($school_id = null) {
        ...
        $this->data['teacherapplicationformval'] = $this->forms->get_teacherapplicationformval($conschoolid,$zonal_id);
    ...
    private function _prepare_teacherapp_validation() {
        ...
        $this->form_validation->set_rules('initial_name_s', $this->lang->line('person1'), 'trim|required');
    ...
    private function _get_teacherapp_data1() {
		...
		//$data['initial_name_e'] = $this->input->post('initial_name_s');
...
//		  echo("dddddddddd");
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    redirect('formskruthy/formsapply/teacherapplicationform' . $data['id']);
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('formskruthy/formsapply/teacherapplicationformedit/' . $this->input->post('schlform_id'));
                }
            } else {
                error($this->lang->line('update_failed'));
        ...
        if ($id) {
            ...
            if (!$this->data['forms']) {
                redirect('formskruthy/formsapply/teacherapplicationform');
            }
        }
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    success($this->lang->line('confirmzonal'));
                     redirect('formskruthy/formsapply/teacherapplicationform' . $data['school_id']);
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('formskruthy/formsapply/confirm/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
...
//        $items[] = 'route_end';
        ...
        $data['cur_status'] = 3;
        $data['kruthyconfirmstatus'] = 2;
        $data['kruthyconfirm_at'] = date('Y-m-d H:i:s');
        $data['kruthyconfirm_by'] = logged_in_user_id();
