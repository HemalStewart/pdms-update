...
class Profile extends My_Controller {
    ...
    public function index() {
        ...
        $department_id= $this->session->userdata('department_id');
             ...
             $director = $this->profile->get_single('users', array('id' => $user_id));
        ...
        $this->data['roles'] = $this->profile->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
         ...
         //   $school = $this->profile->get_school_by_id($this->data['profile']->school_id);            
            $this->data['provincial'] = $this->profile->get_single_provincial($user_id);
            $this->layout->view('profile/provincial', $this->data);
            
        }  elseif ($role_id == ZONAL) {
            
            
         //   $school = $this->profile->get_school_by_id($this->data['profile']->school_id);            
            $this->data['provincial'] = $this->profile->get_single_zonal($user_id);
            $this->layout->view('profile/zonal', $this->data);
            
        }elseif ($role_id == SUBJECT) {
            
            
         //   $school = $this->profile->get_school_by_id($this->data['profile']->school_id);            
            $this->data['provincial'] = $this->profile->get_single_subjectdata($user_id);
            $this->layout->view('profile/subjectdir', $this->data);
            
        }elseif ($role_id == TEACHER) {
            
            $this->layout->view('profile/teacher', $this->data);
            
        }elseif ($this->session->userdata('department_id') == 2) {
            
             $this->data['provincial'] = $this->profile->get_single_directordata($user_id);
            $this->layout->view('profile/director', $this->data);
    ...
    public function update() {
        ...
        if ($_POST) {
            ...
            if ($this->input->post('user_type') == 'student') {
             ...
             if ($this->input->post('user_type') == 'provincial') {
                $updated = $this->profile->update('provincial_dir', $data, array('id' => $this->input->post('id')));
            }
             if ($this->input->post('user_type') == 'zonal') {
                $updated = $this->profile->update('zonal_dir', $data, array('id' => $this->input->post('id')));
            }
            
              if ($this->input->post('user_type') == 'SUBJECT') {
                $updated = $this->profile->update('subject_dir', $data, array('id' => $this->input->post('id')));
            }
...
//            $this->form_validation->set_rules('curpassword', $this->lang->line('currpwd'), 'trim|required|callback_curpwd');
            ...
            $this->form_validation->set_rules('curpassword', $this->lang->line('currpwd'), 'trim|required|callback_curpassword');
    ...
    public function curpassword() {
        
        $curpassword = md5($this->input->post('curpassword'));
      if($curpassword){
            $curpwd = $this->profile->curpassword_check(logged_in_user_id(), $curpassword);
            if ($curpwd) {
               
                return TRUE;
            } else {
                 $this->form_validation->set_message('curpassword', $this->lang->line('invalid_curpwd'));
                return FALSE;
            }
      }
