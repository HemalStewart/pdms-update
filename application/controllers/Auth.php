    ...
    public function login() {
        ...
        if ($_POST) {
            ...
            if (!empty($login)) {
                ...
                $this->session->set_userdata('pirtype', $school->pirtype);

                $this->session->set_userdata('department_id', $login->dep_id);
                ...
                } elseif ($login->role_id == PROVINCIAL) {

                    //  $profile = $this->auth->get_single('provincial_dir', array('user_id' => $login->id)); 

                    $profile = $this->auth->get_single_provincial($login->id);

                    $this->session->set_userdata('provincial_id', $profile->provincial_id);

                    $this->session->set_userdata('provincial_name', $profile->provincialname);
                } elseif ($login->role_id == ZONAL) {

                    //  $profile = $this->auth->get_single('provincial_dir', array('user_id' => $login->id)); 

                    $profile = $this->auth->get_single_zonal($login->id);

                    $this->session->set_userdata('zonal_id', $profile->zonal_id);
                    $this->session->set_userdata('provincial_id', $profile->provincial_id);

                    //        $this->session->set_userdata('provincial_name', $profile->provincialname);
                } elseif ($login->role_id == SUBJECT) {

                    $profile = $this->auth->get_single('subject_dir', array('user_id' => $login->id));
                    
                    $this->session->set_userdata('subject_id', $profile->subject_id);
                } elseif ($login->dep_id == 2) {

                    $profile = $this->auth->get_single('department_dir', array('user_id' => $login->id));
                } else {
                ...
                if (isset($profile->school_id) && $profile->school_id > 0) {
                    $this->session->set_userdata('school_id', $profile->school_id);
                }
                ...
                if (isset($loginrole->name)) {

                    $this->session->set_userdata('rolename', $loginrole->name);
                }

//                if (isset($profile->dep_id)) {
//
//                   $this->session->set_userdata('dep_id', $profile->dep_id);
//
//                }
