...
class Ajax extends My_Controller {
    ...
    public function get_user_by_role() {
        ...
        if ($role_id == SUPER_ADMIN) {
            ...
            $users = $this->ajax->get_list('provincial_dir', array('status' => 1), '', '', '', 'id', 'ASC');
        } elseif ($role_id == ZONAL) {
            $users = $this->ajax->get_list('zonal_dir', array('status' => 1), '', '', '', 'id', 'ASC');
        } elseif ($role_id == SUBJECT) {
            $users = $this->ajax->get_list('subject_dir', array('status' => 1), '', '', '', 'id', 'ASC');
        } elseif ($role_id == DIRECTOR_MINISTRY) {
            $users = $this->ajax->get_list('department_dir', array('status' => 1), '', '', '', 'id', 'ASC');
