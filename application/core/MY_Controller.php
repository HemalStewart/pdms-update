...
class My_Controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->provincial = $this->db->get('provincial')->result();

        $this->privtype = $this->db->get('school_type')->result();

//          $this->district = $this->db->get('district')->result();

//          $this->zonal = $this->db->get('zone')->result();
        ...
        $loginid = $this->session->userdata('id');
         ...
         $profile = $this->db->get_where('users', array('id' => $loginid));
        ...
        // Safely check if school_setting and global_setting exist before using their properties
if (!empty($this->school_setting) && isset($this->school_setting->enable_rtl) && $this->school_setting->enable_rtl == 1) {
    $this->enable_rtl = 1;
} elseif (!empty($this->global_setting) && isset($this->global_setting->enable_rtl) && $this->global_setting->enable_rtl == 1) {
    $this->enable_rtl = 1;
} else {
    $this->enable_rtl = 0;
}
